 ...
 * Adds the `auto` regime-aware strategy from PR #147 and a preamble that
 * predicts each recording's regime from its first 10 snapshots (matching
 * autoAdapter's warmup window).
 */
import { runBacktest } from '../dist/backtest/harness.js';
import { detectRegime } from '../dist/regime.js';
...
(async () => {
  ...
  // Predict regime from first 10 snapshots (matches autoAdapter warmup).
  const predictedRegimes = {};
  for (const rec of RECORDINGS) {
    const window = await readFirstNSnapshots(rec.file, 10);
    predictedRegimes[rec.name] = detectRegime(window);
    process.stderr.write(`[v5 regime] ${rec.name}: predicted=${predictedRegimes[rec.name]} catalog=${CATALOG_LABELS[rec.name]}\n`);
  }
  ...
  console.log('# Strategy Sweep v5 — Raw Results\n');
  console.log(`Total cells: ${cellCount}\n`);
  console.log('## Regime classification accuracy\n');
  console.log('| Recording | Catalog (full-recording) | Predicted (first 10 snapshots) | Match? |');
  console.log('|---|---|---|:--:|');
  for (const rec of RECORDINGS) {
    const cat = CATALOG_LABELS[rec.name];
    const pred = predictedRegimes[rec.name];
    console.log(`| ${rec.name} | ${cat} | ${pred} | ${cat === pred ? '✓' : '✗'} |`);
  }
