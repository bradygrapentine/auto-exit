#!/usr/bin/env node
/**
 * Strategy comparison sweep v3.2 — identical inputs to v3.1, but run AFTER
 * PR #186 changed the harness to subtract cost basis. The reported
 * pnl_cents is now net edge relative to each recording's first-tick mid,
 * not gross sale proceeds.
 *
 * Usage:
 *   node scripts/strategy-sweep-v3.2.mjs > docs/runbooks/<date>-strategy-comparison-v3.2.md
 *
...
(async () => {
  ...
  process.stderr.write(`[v3.2] catalog=${path.basename(catalogPath)}, ${RECORDINGS.length} tradable recordings\n`);

  const byDir = {};
  for (const r of RECORDINGS) byDir[r.dir] = (byDir[r.dir] || 0) + 1;
  process.stderr.write(`[v3.2] by direction: ${JSON.stringify(byDir)}\n\n`);
    ...
    for (const combo of paramCombos) {
      for (const rec of RECORDINGS) {
        ...
        if (cellCount % 200 === 0) {
          ...
          process.stderr.write(`[v3.2] ${cellCount} cells / ${elapsed}s elapsed\n`);
        }
      }
    }
  }
  ...
  process.stderr.write(`\n[v3.2] Ran ${cellCount} cells in ${totalSecs}s.\n\n`);

  // ── Header ─────────────────────────────────────────────────────────────────
  console.log('# Strategy Sweep v3.2 — Expanded Recording Set\n');
