#!/usr/bin/env node
/**
 * Strategy comparison sweep v3.3 — identical inputs to v3.2, but run AFTER
 * PR #189 replaced the queue_aware stub with a real queue-position-tracked
 * fill model. The reported pnl_cents is net edge under realistic passive
 * execution (resting GTC orders pay a queue-position penalty).
 *
 * Usage:
 *   node scripts/strategy-sweep-v3.3.mjs > docs/runbooks/<date>-strategy-comparison-v3.3.md
 *
 * Filters recordings: tradable=✓, dir != dead, snaps >= 100.
 * Breaks "cross-recording winners" out by direction.
 */
...
(async () => {
  ...
  process.stderr.write(`[v3.3] catalog=${path.basename(catalogPath)}, ${RECORDINGS.length} tradable recordings\n`);

  const byDir = {};
  for (const r of RECORDINGS) byDir[r.dir] = (byDir[r.dir] || 0) + 1;
  process.stderr.write(`[v3.3] by direction: ${JSON.stringify(byDir)}\n\n`);
    ...
    for (const combo of paramCombos) {
      for (const rec of RECORDINGS) {
        ...
        if (cellCount % 200 === 0) {
          ...
          process.stderr.write(`[v3.3] ${cellCount} cells / ${elapsed}s elapsed\n`);
        }
      }
    }
  }
  ...
  process.stderr.write(`\n[v3.3] Ran ${cellCount} cells in ${totalSecs}s.\n\n`);

  // ── Header ─────────────────────────────────────────────────────────────────
  console.log('# Strategy Sweep v3.3 — Expanded Recording Set\n');
