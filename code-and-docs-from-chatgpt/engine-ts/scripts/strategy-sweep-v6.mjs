#!/usr/bin/env node
/**
 * Strategy comparison sweep v6 — auto calibration.
 * Sweeps `auto` over warmupTicks × thresholdMode. Reports classification
 * accuracy + pnl per (warmup, mode). Static strategy grid trimmed to a
 * leaderboard subset for context.
 */
import { runBacktest } from '../dist/backtest/harness.js';
import { detectRegime, proportionalThresholds } from '../dist/regime.js';
...
// Trimmed static strategies — just the v4 leaderboard top entries for context.
// Full grid is in v4/v5; v6 focuses cells on auto calibration.
const STRATEGY_GRIDS = [
  { id: 's-aggressive',  paramGrid: {} },
  { id: 'trailing_stop', paramGrid: { trailCents: [10] } },
  { id: 'stop_loss',     paramGrid: { stopPriceCents: [50] } },
  {
    id: 'auto',
    paramGrid: {
      warmupTicks: [10, 50, 100, 200, 500],
      thresholdMode: ['fixed', 'proportional'],
    },
  },
];

// Catalog labels (full-recording direction from 2026-05-08-recording-catalog.md)
const CATALOG_LABELS = {
  'KXINXU (rising +9)':         'rising',
  'KXBTCD (rising +17)':        'rising',
  'KXHIGHNY (rising +16)':      'rising',
  'KXSPOTIFYD (falling -43)':   'falling',
  'KXHIGHCHI (falling -39)':    'falling',
  'KXSPACEXCOUNT (sideways)':   'sideways',
};
...
async function readFirstNSnapshots(file, n) {
  const rl = readline.createInterface({ input: fs.createReadStream(path.join(REC_DIR, file)), crlfDelay: Infinity });
  const out = [];
  for await (const line of rl) {
    ...
    out.push({
      orderbook: {
        yes: (entry.orderbook?.yes ?? []).map(([p, s]) => ({ priceCents: p, size: s })),
        no:  (entry.orderbook?.no  ?? []).map(([p, s]) => ({ priceCents: p, size: s })),
      },
    });
...
(async () => {
  ...
  for (const rec of RECORDINGS) {
    ...
    process.stderr.write(`[v5 cost-basis] ${rec.name}: ${costBases[rec.name]}¢\n`);
  }

  // Predict regime under each (warmupTicks, thresholdMode) — full classification matrix.
  const WARMUPS = [10, 50, 100, 200, 500];
  const MODES = ['fixed', 'proportional'];
  // accuracyMatrix[warmup][mode][recordingName] = predicted label
  const accuracyMatrix = {};
  for (const warmup of WARMUPS) {
    accuracyMatrix[warmup] = {};
    for (const mode of MODES) {
      accuracyMatrix[warmup][mode] = {};
      for (const rec of RECORDINGS) {
        const window = await readFirstNSnapshots(rec.file, warmup);
        const thresholds = mode === 'proportional' ? proportionalThresholds(window.length) : undefined;
        const pred = detectRegime(window, thresholds);
        accuracyMatrix[warmup][mode][rec.name] = pred;
      }
      const matches = Object.entries(accuracyMatrix[warmup][mode]).filter(([rec, pred]) => CATALOG_LABELS[rec] === pred).length;
      process.stderr.write(`[v6 acc] warmup=${warmup} mode=${mode}: ${matches}/${RECORDINGS.length} correct\n`);
    }
  }
  ...
  console.log('# Strategy Sweep v6 — Auto Calibration\n');
  console.log(`Total cells: ${cellCount}\n`);

  console.log('## Classification accuracy matrix (warmup × mode)\n');
  console.log('| warmup | fixed | proportional |');
  console.log('|---:|---:|---:|');
  for (const warmup of WARMUPS) {
    const row = [warmup];
    for (const mode of MODES) {
      const matches = Object.entries(accuracyMatrix[warmup][mode]).filter(([rec, pred]) => CATALOG_LABELS[rec] === pred).length;
      row.push(`${matches}/${RECORDINGS.length}`);
    }
    console.log(`| ${row.join(' | ')} |`);
  }

  console.log('\n## Per-recording predictions (warmup=500, both modes)\n');
  console.log('| Recording | Catalog | warmup=500 fixed | warmup=500 proportional |');
  console.log('|---|---|---|---|');
  for (const rec of RECORDINGS) {
    const fx = accuracyMatrix[500].fixed[rec.name];
    const pr = accuracyMatrix[500].proportional[rec.name];
    const cat = CATALOG_LABELS[rec.name];
    console.log(`| ${rec.name} | ${cat} | ${fx}${fx === cat ? ' ✓' : ' ✗'} | ${pr}${pr === cat ? ' ✓' : ' ✗'} |`);
  }
  console.log('\n## Cost basis per recording (first-tick mid)\n');
