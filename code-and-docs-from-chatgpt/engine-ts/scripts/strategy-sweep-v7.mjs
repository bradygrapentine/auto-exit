#!/usr/bin/env node
/**
 * Strategy comparison sweep v7 — rolling re-classification.
 * Holds warmup=200 fixed (best calibration cell from v6) and sweeps
 * reclassifyInterval ∈ {0, 50, 100, 200} × hysteresisTicks ∈ {3, 5}.
 * Goal: see if mid-recording switching closes the gap to trailing_stop's 5533.
 */
...
const STRATEGY_GRIDS = [
  ...
  { id: 'stop_loss',     paramGrid: { stopPriceCents: [50] } },
  {
    ...
    paramGrid: {
      warmupTicks: [200],
      thresholdMode: ['fixed'],
      reclassifyInterval: [0, 50, 100, 200],
      hysteresisTicks: [3, 5],
    },
  },
];
...
(async () => {
  ...
  // (v7 skips the v6 accuracy preamble — calibration is held at warmup=200 fixed.)
  ...
  console.log('# Strategy Sweep v7 — Auto Rolling Re-classification\n');
  console.log(`Total cells: ${cellCount}\n`);
  console.log('Held: warmup=200, thresholdMode=fixed. Sweep: reclassifyInterval × hysteresisTicks.\n');
